...
📊 ${progressBar(tournament.players.length, tournament.maxPlayers)}
`)
...
`)
  ...
  if (cmd === "1v1") {
    ...
    if (tournament)
      return message.reply("A tournament is already running.");
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.customId === "start") {
    ...
    while (shuffled.length > 0) {
      ...
      if (!p2) {
        interaction.channel.send(`🎉 <@${p1}> gets BYE.`);
